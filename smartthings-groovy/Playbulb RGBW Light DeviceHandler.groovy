...
metadata {
	definition (name: "Playbulb RGBW Light", namespace: "davec1001", author: "David Creager") {
        ...
        attribute "serial_number_string","string"
        attribute "software_revision_string","string"
        attribute "manufacturer_name_string","string"
        attribute "hardware_revision_string", "string"
        attribute "IP","string"
  ...
  /*
	...
	*/ 
		...
		valueTile("firmware", "software_revision_string", width: 2, height: 1) {
    		state "state", label:'Software \r\n${currentValue}'
		}
        valueTile("manufacturer", "manufacturer_name_string", width: 2, height: 1) {
    		state "state", label:'Manufacturer \r\n${currentValue}'
		}
        valueTile("serialno", "serial_number_string", width: 2, height: 1) {
    		state "state", label:'Serial \r\n${currentValue}'
		}
        valueTile("hardware", "hardware_revision_string", width: 2, height: 1) {
    		state "state", label:'Hardware \r\n${currentValue}'
		}

    main "switch"
	//details(["switch", "colorTempSliderControl", "colorTemp", "refresh", "configure", "devIP", "firmware","bridgeIP","colorMode" ]) 
    details(["switch",  "colorTemp", "refresh", "manufacturer", "hardware", "firmware", "serialno", "IP"]) 
   }
}
...
def refresh() {
	log.debug("refresh:" + " apiServerUrl=" + apiServerUrl("parrt") + " ip/port=" +  getDeviceDataByName("ip") + ":" + getDeviceDataByName("port"))
