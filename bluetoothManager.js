...
for (tmp in managerTypes) {
	...
	G_prefixTypes[tmp] = require("./properties.json")[managerTypes[tmp]].AdvertismentPrefixTypes;
}
exports.BluetoothAgent = function (handler) {
	...
	this.connectDevice = function(peripheral,cb) {
		...
		this.connectPeripheral(peripheral, function (error,btBulb) {
			...
			if (peripheral.state == "connected") {
				//console.log("BluetoothAgent:connectDevice: Debug periph=" + peripheral.uuid + " " + 
				...
				if (!btBulb) {
					...
					if (that.numberOfDevices < maxDevices) {
						...
						console.log("BluetoothAgent:connectDevice name=" + peripheral.advertisement.localName + " managerType=" + devType.managerType + " pbType=" + devType.pbType )
	...
	this.getDeviceType = function(peripheralName) {
		...
		for (managerPrefix in managerTypes) {
			if ( (peripheralName) && (peripheralName.substring(0,managerPrefix.length) == managerPrefix)) {
				...
				for (pbPrefix in G_prefixTypes[managerPrefix]) {
					if (peripheralName.substring(0,pbPrefix.length) == pbPrefix) {
						pbType = G_prefixTypes[managerPrefix][pbPrefix];
					}
				}
			}
		}
		...
		if (parsedPrefix.valid) {
			...
			if (!that.peripherals[peripheral.uuid]) {
				...
				that.stopScanningTimer = setTimeout(function(){
							//console.log("BluetoothAgent: on discover: DEBUG executing end to scanning");
				...
				that.connectDevice(that.peripherals[uuid], function(error,pbBulb){
					...
					if (pbBulb) {
						...
						pbBulb.periph.on('servicesDiscover', function (services) {
							console.log("BluetoothAgent: " + pbBulb.periph.advertisement.localName + " uuid=" + pbBulb.periph.uuid + " Services Discovered")
							services.map(function (service) {
								service.on('characteristicsDiscover', function (characteristics) {
									//console.log("BluetoothAgent:DEBUG " + pbBulb.periph.advertisement.localName + " service uuid=" + service.uuid +
